...
export class SyncService extends BaseService {
  ...
  async stream(auth: AuthDto, response: Writable, dto: SyncStreamDto) {
    ...
    const handlers: Record<SyncRequestType, () => Promise<void>> = {
      ...
      [SyncRequestType.FacesV1]: () => this.syncFacesV1(response, checkpointMap, auth),
    };
  ...
  private async syncPeopleV1(response: Writable, checkpointMap: CheckpointMap, auth: AuthDto) {
    ...
    const deletes = this.syncRepository.person.getDeletes(auth.user.id, checkpointMap[deleteType]);
    for await (const { id, ...data } of deletes) {
      send(response, { type: deleteType, ids: [id], data });
    }

    const upsertType = SyncEntityType.PersonV1;
    const upserts = this.syncRepository.person.getUpserts(auth.user.id, checkpointMap[upsertType]);
    for await (const { updateId, ...data } of upserts) {
      send(response, { type: upsertType, ids: [updateId], data });
    }
  }

  private async syncFacesV1(response: Writable, checkpointMap: CheckpointMap, auth: AuthDto) {
    const deleteType = SyncEntityType.FaceDeleteV1;
    const deletes = this.syncRepository.face.getDeletes(auth.user.id, checkpointMap[deleteType]);
    for await (const { id, ...data } of deletes) {
      send(response, { type: deleteType, ids: [id], data });
    }

    const upsertType = SyncEntityType.FaceV1;
    const upserts = this.syncRepository.face.getUpserts(auth.user.id, checkpointMap[upsertType]);
    for await (const { updateId, ...data } of upserts) {
      send(response, { type: upsertType, ids: [updateId], data });
    }
  }
